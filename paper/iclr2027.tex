...
\PairQuestions{} fresh questions it pairs failures by exact original length and
gives each question its partner's uncertainty-chosen origin, so the two arms
carry \emph{identical} origin multisets. The comparison stays null, with
Holm-adjusted $p$-values of \PairHotpotHolm{} and \PairTwoWikiHolm{} and the
two datasets pointing in \emph{opposite} directions
...
Run \PairRunID{} draws \PairQuestions{} fresh Qwen questions, excluding every
identifier used by the earlier studies, and reuses those earlier questions only
as development data for the secondary comparator. Of \PairFailures{} initial
failures, \PairMatched{} pair by exact original length and \PairUnmatched{}
remain as odd stratum remainders. The frozen design fixes
\PairSeeds{} repair seeds, three policies and a \PairFamily{}-test primary
family; execution produced \PairRepairs{} unique repairs and \PairRows{} trial
rows. All originals are generated before any repair, and pairing consumes only
...
\PairHotpotShared{}/\PairHotpotAttempts{} paired attempts on HotpotQA and
\PairTwoWikiShared{}/\PairTwoWikiAttempts{} on 2Wiki. Shared attempts do not
imply equal outcomes elsewhere. The descriptive secondary comparison against
development-fitted random, now fitted on a much larger development failure
sample, agrees with the primary result: \PairHotpotSecondaryDelta{} points on
HotpotQA and \PairTwoWikiSecondaryDelta{} on 2Wiki.

\begin{table}[htbp]
\centering\footnotesize
\caption{Length-matched swap experiment on \PairQuestions{} fresh questions.
...
\PairHotpotHolm{} and \PairTwoWikiHolm{}, with the datasets pointing in
\emph{opposite} directions, \PairHotpotDelta{} percentage points on HotpotQA
and \PairTwoWikiDelta{} on 2Wiki.

The HotpotQA percentile interval excludes zero but should not be read as a
positive effect: only \PairHotpotPositive{} of \PairHotpotPairs{} pairs are
non-zero blocks, and an exact two-sided sign-flip test cannot report below
$2/2^{m}$ for $m$ non-zero blocks, a floor of \PairHotpotFloor{} here. That
